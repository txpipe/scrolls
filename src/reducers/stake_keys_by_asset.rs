...
impl Reducer {
    fn config_key(&self, subject: String, epoch_no: u64) -> String {
        let def_key_prefix = "stake_keys_by_asset";
            ...
            match asset {
                Asset::NativeAsset(policy_id, _, quantity) => {
                    if self.is_policy_id_accepted(&policy_id) {
                        ...
                        let crdt =
                            model::CRDTCommand::SortedSetRemove(key, stake_address.clone(), delta);
            ...
            match asset {
                Asset::NativeAsset(policy_id, _, quantity) => {
                    if self.is_policy_id_accepted(&policy_id) {
                        ...
                        let crdt =
                            model::CRDTCommand::SortedSetAdd(key, stake_address.clone(), delta);
...
impl Config {
    pub fn plugin(
        ...
        super::Reducer::StakeKeysByAsset(reducer)
    }
}
